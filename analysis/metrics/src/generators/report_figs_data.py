...
ax1.set_title(ptitle("(а) Распределение длин")); ax1.legend(); ax1.grid(axis="x",alpha=0)
...
ax2.set_xlabel("число белков"); ax2.set_title(ptitle("(б) Самые частые роды организмов")); ax2.grid(axis="y",alpha=0)
fig.suptitle(ptitle("Распределение длин сегментов и родов организмов в датасете 2026 года"),fontsize=14,weight="bold",color=INK,y=1.02)
fig.tight_layout(rect=[0,0,1,0.95])
for d in outdirs(): fig.savefig(d+"data_distributions.png",bbox_inches="tight")
print("data_distributions done", "(PRES)" if PRES else "")
...
ax1.set_ylabel("доля сегментов, %"); ax1.set_title(ptitle("(а) Профиль длин по фолдам")); ax1.legend(ncol=2,fontsize=8); ax1.grid(axis="x",alpha=0)
...
ax2.set_xlabel("отклонение от общего профиля (L1)"); ax2.set_title(ptitle("(б) Отклонение профиля длин фолда от общего")); ax2.grid(axis="y",alpha=0)
fig.suptitle(ptitle("Распределение длин сегментов по фолдам и его отклонение от общего по датасету"),fontsize=14,weight="bold",color=INK,y=1.02)
fig.tight_layout(rect=[0,0,1,0.95])
for d in outdirs(): fig.savefig(d+"fold_divergence.png",bbox_inches="tight")
print("fold_divergence done", "(PRES)" if PRES else "")
