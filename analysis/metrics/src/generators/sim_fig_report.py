...
LAB={"baseline_esm2":"ESM-2 (база)","esmc6b_3di_zeroctrl":"ESM-C 6B + boundary (без 3Di)",
     "esmc6b_3di_gated_boundary":"ESM-C 6B + boundary + 3Di"}
COL={"baseline_esm2":"#9aa3ad","esmc6b_3di_zeroctrl":"#9a78c2","esmc6b_3di_gated_boundary":"#4ca37a"}
...
ax1.set_title("(а) Полнота против представленности рода\n"+r"$\rho_{Спирмена}$: "+", ".join(f"{rhos[m]:+.2f}" for m in MODELS))
...
ax2.set_title("(б) Полнота против похожести сегмента к обучающим\n(Δ — прирост от наименее к наиболее похожим)")
fig.suptitle("Что определяет полноту: представленность рода (а) против похожести к обучающим (б)\n"
...
fig.savefig(FIG+"similarity.png",bbox_inches="tight"); fig.savefig(OUT+"similarity.png",bbox_inches="tight")
print("wrote similarity.png (3 models, all segments)")
