...
Proteolysis cuts precursor proteins into mature peptides and propeptides.Predicting the products has direct value in applied filds. 
...
A precursor $x=(a_1,\dots,a_L)$ is labelled per residue with
$y_t \in \{\textsc{None},\textsc{Peptide},\textsc{Propeptide}\}$; contiguous runs
of a label form typed segments. A predicted segment is a true positive if both
endpoints fall within $\pm\tau$ residues of a ground-truth segment of the same
type. The field default is $\tau=3$; we report a curve in $\tau$ instead
...
%\textbf{From 2022 to 2026.} 
The original DeepPeptide dataset was built from \texttt{PEPTIDE} and \texttt{PROPEP} annotations in the 2022 Swiss-Prot release. We rebuilt the dataset with the same methodology on the 2026 release~\citep{uniprot2025}. The data is heavily heterogeneous: the 2026 set spans more than a thousand genera, from human, mouse, and rat to plants and venomous animals. %such as cone snails (Conus, the single most frequent genus, $\approx$900 proteins) and spiders (Cyriopagopus, Lycosa). 
Peptide and propeptide segment lengths are similarly unevenly distributed.% (\Cref{fig:datadist}). 
This heterogeneity matters for evaluation: different families are predicted with markedly different quality, which is part of why a single held-out fold is an unreliable comparison unit (\Cref{sec:app-old-protocol}). \Cref{tab:dataset} summarizes the resulting change in scale.
...
We retain peptides and propeptides of length 5--50 residues. The lower bound ensures meaningful evaluation under the $\pm3$ residue matching tolerance (\Cref{sec:matching}), while the upper bound covers the vast majority of the length distribution and matches the maximum segment length supported by the CRF decoder (\Cref{sec:intro}); both cutoffs are retained from the original methodology for comparability.

To prevent information leakage from homologous proteins, folds are constructed using GraphPart~\citep{graphpart2023} with a 30\% sequence-identity threshold. To further avoid biases in cleavage-site composition, cleavage motifs are encoded by ESM-2 embeddings of the two residues flanking each segment boundary on either side. The embeddings are clustered into 50 motif classes using $k$-means, and each protein is assigned its dominant motif class. These labels are jointly used by GraphPart to balance motif composition across folds while preserving the homology constraint.
