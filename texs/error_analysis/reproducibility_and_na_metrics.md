...
2. **Embedding content changed between training and now (structural runs).** The
   AFT / 3Di structural embeddings were (re)generated with partial protein coverage
   during the data reorg, so a fresh inference evaluates a slightly different residue
   set than training did. This is large for AFT (drift up to 0.14).
