...
Nothing was selected on outer-fold scores inside confirmation, since every cell of the 2×2 is reported, but the screening that chose the candidates (Appendix&nbsp;F) ran on the same proteins, and sealing that would take a third held-out level at roughly 1,600 GPU-hours for the grid, on 2/5 of the data. The spread covers fold composition, not seed variation, and the base row is untuned rather than the published system (Appendix&nbsp;E).
