...
DeepPeptide reports precision 0.68 and recall 0.49 at ±3, both segment types together, averaged over the twenty models of its nested cross-validation on the 2022 data, which implies an F1 of 0.570. Our base architecture on that release scores 0.574±0.069 under the same protocol and the same matcher. The F1 reproduces, the operating point does not, and Appendix&nbsp;D gives both the numbers and the reason: the published system tunes its hyperparameters per outer fold, while we hold one configuration fixed across all 20 cells so that every row of Table&nbsp;1 is measured under the same setting.
...
A zero-initialized boundary head at the decoder and a lightweight adapter at the input improve segment F1 by +0.054 together on ESM-2 and +0.079 on ESM-C 6B, and they combine because they act on different errors. What matters for a design loop is where the gain comes from, and the two blocks answer differently. At a gate asking only for one residue of overlap the head covers less than the base while the adapter covers more, so the head’s contribution is placement alone and the adapter’s is placement on top of coverage. Either way the part that grows as the requirement tightens is placement, reaching +0.077±0.005 at exact match on ESM-2 and +0.112±0.013 on ESM-C 6B. An oracle used to decide whether a construct yields the peptide it was designed to yield is used at exactly that end of the curve, and it is the end the benchmark’s headline number reports least well.
